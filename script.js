...
// ---------------------------------------------------------------------------
// Sidebar nav icons (Lucide-style line icons, one per doc page) — gives the
// docs sidebar the same icon+label look as the app dashboard.
// ---------------------------------------------------------------------------
const NAV_ICONS = {
  zap: '<polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/>',
  grid: '<rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/>',
  key: '<path d="M21 2l-2 2m-7.6 7.6a5.5 5.5 0 1 1-7.78 7.78 5.5 5.5 0 0 1 7.78-7.78zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3"/>',
  ...
  transfer: '<polyline points="15 14 20 9 15 4"/><path d="M4 20v-7a4 4 0 0 1 4-4h12"/>',
...
const NAV_ICON_FOR = {
  '/get-started/quick-start': 'zap', '/get-started/dashboard-overview': 'grid', '/get-started/api-keys': 'key',
  '/agents/create-agent': 'plus', '/agents/agent-settings': 'sliders', '/agents/voice-configuration': 'mic',
  '/agents/test-agent': 'play', '/agents/agent-tools': 'tool', '/agents/agent-knowledge-base': 'book',
  '/agents/agent-mcp': 'server', '/agents/agent-privacy': 'shield', '/agents/costum-variables': 'code',
  '/agents/embed-agent': 'globe', '/agents/llm-settings': 'cpu', '/agents/observer-agent': 'eye', '/agents/advanced-settings': 'settings',
  '/phone/add-sip-provider': 'server', '/phone/add-phone-number': 'phone', '/phone/call-routing': 'shuffle', '/phone/call-logs': 'list',
  '/contact-center/overview': 'headphones', '/contact-center/agent-guide': 'user', '/contact-center/groups': 'users',
  '/contact-center/members-roles': 'userCheck', '/contact-center/role-mapping': 'shuffle', '/contact-center/inbound-sip-routing': 'transfer',
  '/contact-center/ring-strategy': 'bell', '/contact-center/wait-hold': 'clock', '/contact-center/queue-settings': 'layers',
  '/contact-center/reporting': 'barChart', '/contact-center/call-transfer': 'transfer',
  '/campaigns/create-campaign': 'radio', '/campaigns/dial-lists': 'list', '/campaigns/campaign-analytics': 'pie', '/campaigns/do-not-call': 'slash',
...
function injectSidebarIcons(container) {
  if (!container) return;
  container.querySelectorAll('a.nav-link[data-href]').forEach(a => {
    if (a.querySelector('.nav-icon')) return;
    const key = NAV_ICON_FOR[a.getAttribute('data-href')] || 'dot';
    const svg = `<svg class="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${NAV_ICONS[key]}</svg>`;
...
function updateSidebarForTab(route) {
  ...
  if (route === '/api-reference') {
    ...
    `;
      ...
      // Add a line icon before each nav item (dashboard-style)
      injectSidebarIcons(sidebar);
